...
const getCourseDetails = async (req, res) => {
    try {
        ...
        //find course details
        const courseDetails = await CourseModel.find({ _id: updateSearchIndex }).populate(
            {
                path: "instructor",
                populate: {
        ...
        if (!courseDetails) {
            return res.json({
                success: false,
                message: "Could not find the course with the provided courseId",
            })
        }
        ...
        return res.json({
            success: false,
            message: "Something wrong while fetching courseDetails!"
        })
    }
}
